...
// TODO Add here the import of Group
...
class FSDirectory implements FSConstants, Closeable {
	...
	public FSDirectory(FSImage fsImage, FSNamesystem ns, Configuration conf)
			throws IOException {
		rootDir = new INodeDirectoryWithQuota(INodeDirectory.ROOT_NAME, ns
	...
	void loadFSImage(Collection<File> dataDirs, StartupOption startOpt)
			throws IOException {
   ...
   */
	 ...
	 */
	Block addBlock(String path, INode file, Block block, int n, int m)
			throws IOException {
		waitForReady();
		...
		synchronized (rootDir) {
			...
			fileNode.addBlock(blockInfo, n, m);
		...
		synchronized (rootDir) {
			...
			if (dstInodes[dstInodes.length - 1] != null) { // check if
				...
				try {
					...
					dstChild = addChild(dstInodes, dstInodes.length - 1,
							srcChild, false);
	 ...
	 * @param deltaCount
	 *            the delta change of the count
	 * @throws QuotaExceededException
	 *             if the new count violates any quota limit
	 */
	private static void updateCount(INode[] inodes, int numOfINodes,
			long deltaCount) throws QuotaExceededException {
		if (numOfINodes > inodes.length) {
			numOfINodes = inodes.length;
		}
		// check existing components in the path
		List<INodeDirectoryWithQuota> inodesWithQuota = new ArrayList<INodeDirectoryWithQuota>(
				numOfINodes);
		int i = 0;
		try {
			for (; i < numOfINodes; i++) {
   ...
   */
	...
	private void unprotectedMkdir(INode[] inodes, int pos, byte[] name,
		...
		inodes[pos] = addChild(inodes, pos, new INodeDirectory(name,
				permission, timestamp), inheritPermission);
	}
	...
	private <T extends INode> T addChild(INode[] pathComponents, int pos,
			T child, boolean inheritPermission) throws QuotaExceededException {
	...
	private INode removeChild(INode[] pathComponents, int pos)
		...
		if (removedNode != null) {
			updateCount(pathComponents, pos, -removedNode.numItemsInTree());
		}
		return removedNode;
	}
   ...
   */
	 ...
	 * 
	 * @throws QuotaExceededException
	 *             if the count update violates any quota limitation
	 */
	 ...
	 * @throws QuotaExceededException
	 *             if the count is greater than its quota
	 */
	private static long updateCountForINodeWithQuota(INode node)
			throws QuotaExceededException {
		long count = 1L;
		if (node.isDirectory()) {
		...
		synchronized (rootDir) {
			...
			if (targetNode == null) {
				...
				if (dirNode instanceof INodeDirectoryWithQuota) {
					...
					// a non-quota directory; so replace it with a directory
					// with quota
	 ...
	 */
	void setQuota(String src, long quota) throws FileNotFoundException,
			QuotaExceededException {
