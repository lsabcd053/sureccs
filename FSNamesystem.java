...
import org.apache.hadoop.dfs.RSGroup;
...
class FSNamesystem implements FSConstants, FSNamesystemMBean {
	public static final Log LOG = LogFactory
			.getLog("org.apache.hadoop.fs.FSNamesystem");
	...
	public static final Log auditLog = LogFactory
			.getLog("org.apache.hadoop.fs.FSNamesystem.audit");
	 ...
	 */
	SortedMap<String, DatanodeDescriptor> datanodeMap = new TreeMap<String, DatanodeDescriptor>();
	...
	private LinkedBlockingQueue<DatanodeDescriptor> resolutionQueue = new LinkedBlockingQueue<DatanodeDescriptor>();
	...
	// TODO Add the encoding arguments
	private int RSn;
	private int RSm;
	...
	private void initialize(NameNode nn, Configuration conf) throws IOException {
		...
		this.hostsReader = new HostsFileReader(conf.get("dfs.hosts", ""), conf
				.get("dfs.hosts.exclude", ""));
		...
		this.dnsToSwitchMapping = (DNSToSwitchMapping) ReflectionUtils
				.newInstance(conf.getClass("topology.node.switch.mapping.impl",
		...
		sslConf
				.addResource(conf
						.get("https.keystore.info.rsrc", "sslinfo.xml"));
		String keyloc = sslConf.get("https.keystore.location");
		if (null != keyloc) {
			this.infoServer.addSslListener(secInfoSocAddr, keyloc, sslConf.get(
					"https.keystore.password", ""), sslConf.get(
		...
		this.infoServer.addServlet("getimage", "/getimage",
				GetImageServlet.class);
		this.infoServer.addServlet("listPaths", "/listPaths/*",
	...
	private void setConfigurationParameters(Configuration conf)
		...
		this.defaultPermission = PermissionStatus.createImmutable(fsOwner
				.getUserName(), supergroup, new FsPermission(filePermission));

		this.replicator = new ReplicationTargetChooser(conf.getBoolean(
				"dfs.replication.considerLoad", true), this, clusterMap);

		// TODO This should be turn into (n,m),
		// And n, m should be configured by clients
		// We have the rules that n-m+m/2 less than the whole machines in the
		// current local rack
		// And 2*(n-m)+m should be less than all the nodes we have
	...
	NamespaceInfo getNamespaceInfo() {
		return new NamespaceInfo(dir.fsImage.getNamespaceID(), dir.fsImage
				.getCTime(), getDistributedUpgradeVersion());
	}
	...
	synchronized BlocksWithLocations getBlocks(DatanodeID datanode, long size)
		...
		return new BlocksWithLocations(results
				.toArray(new BlockWithLocations[results.size()]));
	}
	...
	private long addBlock(Block block, List<BlockWithLocations> results) {
		ArrayList<String> machineSet = new ArrayList<String>(blocksMap
				.numNodes(block));
	...
	public synchronized void setPermission(String src, FsPermission permission)
		...
		if (auditLog.isInfoEnabled()) {
			...
			logAuditEvent(UserGroupInformation.getCurrentUGI(), Server
					.getRemoteIp(), "setPermission", src, null, stat);
		}
	}
	...
	public synchronized void setOwner(String src, String username, String group)
		...
		if (auditLog.isInfoEnabled()) {
			final FileStatus stat = dir.getFileInfo(src);
			logAuditEvent(UserGroupInformation.getCurrentUGI(), Server
					.getRemoteIp(), "setOwner", src, null, stat);
		}
	}
	...
	LocatedBlocks getBlockLocations(String src, long offset, long length)
		...
		final LocatedBlocks ret = getBlockLocationsInternal(dir
				.getFileINode(src), offset, length, Integer.MAX_VALUE);
		if (auditLog.isInfoEnabled()) {
			logAuditEvent(UserGroupInformation.getCurrentUGI(), Server
					.getRemoteIp(), "open", src, null, null);
		}
		return ret;
	}

	private synchronized LocatedBlocks getBlockLocationsInternal(
			INodeFile inode, long offset, long length, int nrBlocksToReturn) {
		if (inode == null) {
			return null;
		}
		...
		if (blocks.length == 0) {
			return new LocatedBlocks(inode, new ArrayList<LocatedBlock>(
					blocks.length));
		}
		...
		do {
			...
			if (numCorruptNodes != numCorruptReplicas) {
				LOG
						.warn("Inconsistent number of corrupt replicas for "
								+ blocks[curBlk] + "blockMap has "
								+ numCorruptNodes
								+ " but corrupt replicas map has "
								+ numCorruptReplicas);
			}
	...
	public boolean setReplication(String src, short replication)
		...
		if (status && auditLog.isInfoEnabled()) {
			logAuditEvent(UserGroupInformation.getCurrentUGI(), Server
					.getRemoteIp(), "setReplication", src, null, null);
		}
		return status;
	}
	...
	void startFile(String src, PermissionStatus permissions, String holder,
		...
		if (auditLog.isInfoEnabled()) {
			final FileStatus stat = dir.getFileInfo(src);
			logAuditEvent(UserGroupInformation.getCurrentUGI(), Server
					.getRemoteIp(), "create", src, null, stat);
		}
	}
	...
	private synchronized void startFileInternal(String src,
			...
			throws IOException {
		NameNode.stateChangeLog.debug("DIR* NameSystem.startFile: file " + src
		...
		try {
			...
			if (myFile != null && myFile.isUnderConstruction()) {
				...
				//
				Lease lease = leaseManager.getLease(new StringBytesWritable(
						holder));
				//
	...
	void appendFileInternal(String src, String holder, String clientMachine)
		...
		try {
			...
			// assume f != null && !f.isUnderConstruction() && lease does not
			// exist
			// TODO: remove the assumption

			DatanodeDescriptor clientNode = host2DataNodeMap
					.getDatanodeByHost(clientMachine);
			INodeFileUnderConstruction newnode = f
					.toINodeFileUnderConstruction(holder, clientMachine,
							clientNode);
	...
	private INodeFileUnderConstruction checkLease(String src, String holder)
		...
		if (file == null) {
			Lease lease = leaseManager
					.getLease(new StringBytesWritable(holder));
	...
	private synchronized int completeFileInternal(String src, String holder)
			throws IOException {
	...
	private Block allocateBlock(String src, INode file) throws IOException {
		...
		//TODO Code add here
		int blkSize = filenode.getBlockSize();
		
		// It's the very time to generate the redundant block,
		// Because blocks in the group reaches RSm, that's the 
		// original number of block for coding
		if(blkSize % this.RSm == 0)
		{		
			addCodingBlockToGroup(file, blkSize);		
		}
		//TODO Add Complete
	...
	/*
	 *  Here we just add the latest block to the last group.
	 *  At the mean time, we will figure out if it is the 
	 *  very time to generate the redundant block
	 */
	public void addCodingBlockToGroup(INode file, int blkSize) throws IOException
	{
		// TODO Add the code to add a block into the group.
		// Here, when the current block is ready for write,
		// the previous block has already been written for sure.
		// So every time a new block was generated, the previous
		// one could be added to group
		int n = this.RSn;
		int m = this.RSm;
		Block blocks[] = new Block[(n-m)];
		BlockInfo newCodingBlocks[] = new BlockInfo[(n - m)];
		
		INodeFile filenode = (INodeFile) file;	
		RSGroup lastGroup = filenode.getLastGroup(); 
		if(lastGroup == null)
			return;
		
		if (blkSize != 0) { // When there is no block yet
			int numCurBlocksInLastGroup = lastGroup.getBlockSize();
			lastGroup.setNumOfRealBlocks(numCurBlocksInLastGroup);
			if(numCurBlocksInLastGroup < m) { // Add some dirty blocks
				for(int i = 0; i < (m - numCurBlocksInLastGroup); i++){
					BlockInfo b = new BlockInfo(new Block(0, 0, 0), 0);
					lastGroup.addBlock(b);
				}
			}
			for (int i = 0; i < (n - m); i++) {
				do {
	...
	private synchronized void dumpRecentInvalidateSets(PrintWriter out) {
		...
		out.println("Metasave: Blocks waiting deletion from " + size
				+ " datanodes.");
		if (size == 0) {
			return;
		}
	...
	public synchronized void markBlockAsCorrupt(Block blk, DatanodeInfo dn)
		...
		if (!blocksMap.contains(blk, node)) {
			...
			// sent by the Datanode on startup
			NameNode.stateChangeLog
	...
	public boolean renameTo(String src, String dst) throws IOException {
		...
		if (status && auditLog.isInfoEnabled()) {
			...
			logAuditEvent(UserGroupInformation.getCurrentUGI(), Server
					.getRemoteIp(), "rename", src, dst, stat);
		}
		return status;
	}
	...
	public boolean delete(String src, boolean recursive) throws IOException {
		...
		if (status && auditLog.isInfoEnabled()) {
			logAuditEvent(UserGroupInformation.getCurrentUGI(), Server
					.getRemoteIp(), "delete", src, null, null);
		}
		return status;
	}
	 ...
	 */
	synchronized boolean deleteInternal(String src, boolean enforceSafeMode,
	...
	public boolean mkdirs(String src, PermissionStatus permissions)
		...
		if (status && auditLog.isInfoEnabled()) {
			final FileStatus stat = dir.getFileInfo(src);
			logAuditEvent(UserGroupInformation.getCurrentUGI(), Server
					.getRemoteIp(), "mkdirs", src, null, stat);
		}
		return status;
	}
	 ...
	 * @param quota
	 *            The limit of the number of names in or below the directory
	 * @throws IOException
	 *             if the path is not a directory or the number of existing
	 *             names in or below the directory is greater than the given
	 *             quota
	 */
	...
	private void finalizeINodeFileUnderConstruction(String src,
		...
		String s_INF_UC = "In INodeFileUnderConstruction, " // + pendingFile.toString()
						  + "\n" +
						  "blocks = \n";
		for (BlockInfo b : pendingFile.getBlocks())
			s_INF_UC = s_INF_UC + b.toString() + " \n";
		s_INF_UC = s_INF_UC + "\n" + "groups = \n";
		...
		// TODO SUR_ECCS.log <function:"Compare the variables in INodeFileUnderConstruction and INodeFile in file "+this.toString()>
		String s_INF = "In INodeFile, "// + newFile.toString()
						+ "\n" +
					   "blocks = \n";
		for (BlockInfo b : newFile.getBlocks())
			s_INF = s_INF + b.toString() + " \n";
		s_INF = s_INF + "\n" + "groups = \n";
	...
	synchronized void commitBlockSynchronization(Block lastblock,
		...
		LOG
				.info("commitBlockSynchronization(lastblock=" + lastblock
		...
		if (!closeFile) {
			...
			getEditLog().logSync();
			LOG
					.info("commitBlockSynchronization(" + lastblock
							+ ") successful");
			return;
		}
	...
	public DFSFileInfo[] getListing(String src) throws IOException {
		...
		if (auditLog.isInfoEnabled()) {
			logAuditEvent(UserGroupInformation.getCurrentUGI(), Server
					.getRemoteIp(), "listStatus", src, null, null);
		}
	...
	private class ResolutionMonitor implements Runnable {
		public void run() {
			try {
				while (fsRunning) {
					try {
						List<DatanodeDescriptor> datanodes = new ArrayList<DatanodeDescriptor>(
								resolutionQueue.size());
						// Block if the queue is empty
						datanodes.add(resolutionQueue.take());
						resolutionQueue.drainTo(datanodes);
						List<String> dnHosts = new ArrayList<String>(datanodes
								.size());
						for (DatanodeDescriptor d : datanodes) {
							dnHosts.add(d.getName());
						}
						List<String> rName = dnsToSwitchMapping
								.resolve(dnHosts);
						if (rName == null) {
						...
						FSNamesystem.LOG
								.debug("ResolutionMonitor thread received "
	...
	/**
	 * Has the block report of the datanode represented by nodeReg been
	 * processed yet.
	 * 
	 * @param nodeReg
	...
	synchronized boolean isResolved(DatanodeRegistration dnReg) {
		try {
			return !getDatanode(dnReg).getNetworkLocation().equals(
					NetworkTopology.UNRESOLVED);
		} catch (IOException ie) {
			return false;
		}
	}
	...
	public synchronized void registerDatanode(DatanodeRegistration nodeReg)
		...
		DatanodeID dnReg = new DatanodeID(dnAddress + ":" + nodeReg.getPort(),
				nodeReg.getStorageID(), nodeReg.getInfoPort(), nodeReg
						.getIpcPort());
		...
		DatanodeDescriptor nodeDescr = new DatanodeDescriptor(nodeReg,
				NetworkTopology.UNRESOLVED, hostName);
	...
	DatanodeCommand handleHeartbeat(DatanodeRegistration nodeReg,
		...
		synchronized (heartbeats) {
			synchronized (datanodeMap) {
				...
				// TODO Add command for encoding
				if(cmd == null){
					// Cause we don't know the opration conditions for 
					// coding task, we just currently support only one 
					// coding task processing at one time.
					cmd = nodeinfo.getEncodingCommand();
				}
				
				// TODO Add command for decoding
				if(cmd == null) {
					cmd = nodeinfo.getDecodingCommand();
				}
			}
		}
	...
	private int computeEncodingWork(int groupsToProcess) {
		...
		synchronized(neededEncodedGroups){
			...
			groupsToProcess = 1; // We currently support only one group per time
	 ...
	 * 
	 */
	private void initiateEncodingProcess(RSGroup group, int workFound)
	{
		...
		int numRealSources = group.getNumOfRealBlocks();
		...
		Debug.writeDebug("The real number of sources to read the block is:"
				+ numRealSources);
		for(i = 0; i < m; i++)
		{
			if (i < numRealSources) {
				...
				sources[i] = new DatanodeDescriptor((new DatanodeID(
						"NullForCode", "0", 0, 0)));
				Debug.writeDebug("We have an empty block of position-" + i
						+ " in this group!");
			}
		}
	 ...
	 */
	private synchronized int computeReplicationWork(int blocksToProcess)
			throws IOException {
		...
		synchronized (neededReplications) {
			...
			Iterator<Block> neededReplicationsIterator = neededReplications
					.iterator();
			...
			for (int blkCnt = 0; blkCnt < blocksToProcess; blkCnt++, replIndex++) {
				...
				List<DatanodeDescriptor> containingNodes = new ArrayList<DatanodeDescriptor>();
				NumberReplicas numReplicas = new NumberReplicas();
				DatanodeDescriptor srcNode = chooseSourceDatanode(block,
						containingNodes, numReplicas);
				if (srcNode == null) {
					// block can not be replicated from any node
					// TODO but we can still recover it by coding method
					processDecodingTask(block,neededReplicationsIterator,
				...
				int maxTargets = maxReplicationStreams
						- srcNode.getNumberOfBlocksToBeReplicated();
				assert maxTargets > 0 : "Datanode "
						+ srcNode.getName()
						+ " should have not been selected as a source for replication.";
				DatanodeDescriptor targets[] = replicator.chooseTarget(Math
						.min(requiredReplication - numEffectiveReplicas,
								maxTargets), srcNode, containingNodes, null,
						block.getNumBytes());
				if (targets.length == 0)
					continue;
	...
	/** TODO
	 *It is necessary to process coding task when a block lost all its
	 * replication and it could be recover from coding method by the other
	 * blocks in the same group
	 * @param block The broken block to be recovered from erasure decoding
	 * @param neededReplicationsIterator The neededReplications list.</br>
	 * After recovery processed, the task should be removed from needeReplications list,</br>
	 * and meanwhile be added to the pendingReplications list
	 * @param workFound The total work found during an assignation of the recover task
	 */
	private void processDecodingTask(Block block,
						   ...
						   int workFound)
	{
		...
		Debug.writeDebug("The number of real sources is:" + numRealSources);
		DatanodeDescriptor sources[] = new DatanodeDescriptor[n];
		// Choose the sourceNode from the other blocks in the same group
		List<DatanodeDescriptor> containingNodes = new ArrayList<DatanodeDescriptor>();
		NumberReplicas numReplicas = new NumberReplicas();
		
		for(i = 0; i < n ; i++){
			if (i < numRealSources) { // The first real sources
				...
				if (sources[i] == null) {
					...
					Debug.writeDebug("The block:" + grpBlocks[i]
							+ " could be got from source:" + sources[i]);
				}
			} else if(i >= numRealSources && i < m) { // We add some null sources for decoding
				sources[i] = new DatanodeDescriptor((new DatanodeID(
						"NullForCode", "0", 0, 0)));
				Debug.writeDebug("We have an empty block of position-" + i
						+ " in this group!");
			} else if(i >= m) { // Final is the redundant sources
				sources[i] = chooseSourceDatanode(grpBlocks[i],
						containingNodes, numReplicas);
				if (sources[i] == null) {
					blockDamaged++;
		...
		if (targets.length >= requiredReplication) {
			neededReplicationsIterator.remove(); // remove from
													// neededReplications
			replIndex--;
			pendingReplications.add(block, targets.length);
	...
	public synchronized void processReport(DatanodeID nodeID,
		...
		if (node.getNetworkLocation().equals(NetworkTopology.UNRESOLVED)) {
			LOG
					.info("Ignoring block report from "
							+ nodeID.getName()
   ...
   */
			...
			if (fileINode != null) {
				...
				if (curExpectedReplicas > curReplicas) {
					...
					if (!neededReplications.contains(block)
						...
						// started.
						//
						neededReplications.update(block, curReplicas, num
								.decommissionedReplicas(), curExpectedReplicas,
								-1, 0);
					}
				}
	...
	private boolean inHostsList(DatanodeID node) {
		...
		return (hostsList.isEmpty() || hostsList.contains(node.getName())
				|| hostsList.contains(node.getHost()) || ((node instanceof DatanodeInfo) && hostsList
	...
	private boolean inExcludedHostsList(DatanodeID node) {
		...
		return (excludeList.contains(node.getName())
				|| excludeList.contains(node.getHost()) || ((node instanceof DatanodeInfo) && excludeList
				.contains(((DatanodeInfo) node).getHostName())));
	}

	/**
	 * Rereads the files to update the hosts and exclude lists. It checks if any
	 * of the hosts have changed states: 1. Added to hosts --> no further work
	 * needed here. 2. Removed from hosts --> mark AdminState as decommissioned.
	 * 3. Added to exclude --> start decommission. 4. Removed from exclude -->
	 * stop decommission.
	 */
	 ...
	 */
	public synchronized boolean verifyNodeRegistration(
	...
	class SafeModeInfo {
		...
		boolean isConsistent() throws IOException {
			...
			for (Iterator<Collection<Block>> it = recentInvalidateSets.values()
					.iterator(); it.hasNext();) {
	...
	PermissionStatus createFsOwnerPermissions(FsPermission permission) {
		return new PermissionStatus(fsOwner.getUserName(), supergroup,
				permission);
	}

	private PermissionChecker checkOwner(String path)
			throws AccessControlException {
	...
	private void checkSuperuserPrivilege() throws AccessControlException {
		if (isPermissionEnabled) {
			PermissionChecker pc = new PermissionChecker(fsOwner.getUserName(),
					supergroup);
			if (!pc.isSuper) {
				throw new AccessControlException(
	 ...
	 * .
	 */
	private PermissionChecker checkPermission(String path,
	 ...
	 * Increments, logs and then returns the stamp
	 */
	synchronized long nextGenerationStampForBlock(Block block)
			throws IOException {
		BlockInfo storedBlock = blocksMap.getStoredBlock(block);
		if (storedBlock == null) {
	...
	void saveFilesUnderConstruction(DataOutputStream out) throws IOException {
		synchronized (leaseManager) {
			...
			for (Lease lease : leaseManager.getSortedLeases()) {
				...
				for (Iterator<StringBytesWritable> i = files.iterator(); i
						.hasNext();) {
