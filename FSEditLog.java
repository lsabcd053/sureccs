...
class FSEditLog {
		...
		void close() throws IOException {
			...
			if (getBufSize() != 0) {
				throw new IOException("FSEditStream has " + getBufSize()
						+ " bytes still to be flushed and cannot " + "closed.");
			}
			od.close();
			fp.close();
			buf1 = buf2 = null;
	...
	synchronized void processIOError(int index) {
		if (editStreams == null || editStreams.size() <= 1) {
			FSNamesystem.LOG
					.fatal("Fatal Error : All storage directories are inaccessible.");
	...
	int loadFSEdits(File edits) throws IOException {
		...
		String path = null;
		int numOpAdd = 0, numOpClose = 0, numOpDelete = 0, numOpRename = 0, numOpSetRepl = 0, numOpMkDir = 0, numOpSetPerm = 0, numOpSetOwner = 0, numOpSetGenStamp = 0, numOpOther = 0;
		...
		if (edits != null) {
			...
			try {
				...
				if (available) {
					...
					if (logVersion < FSConstants.LAYOUT_VERSION) // future
																	// version
				...
				assert logVersion <= Storage.LAST_UPGRADABLE_LAYOUT_VERSION : "Unsupported version "
						+ logVersion;
				...
				while (true) {
					...
					case OP_CLOSE: {
						...
						if (-7 == logVersion && length != 3 || logVersion < -7
								&& length != 4) {
						...
						// Older versions of HDFS does not store the block size
						// in inode.
						// If the file has more than one block, use the size of
						// the
						// first block as the blocksize. Otherwise use the
						// default
						// block size.
						if (-8 <= logVersion && blockSize == 0) {
							if (blocks.length > 1) {
								...
								long first = ((blocks.length == 1) ? blocks[0]
										.getNumBytes() : 0);
								blockSize = Math.max(fsNamesys
										.getDefaultBlockSize(), first);
							}
						}
						
						// TODO Generally we use the default action
						// Cause I do not really understand what the version means
						// TODO
						// TODO
						// TODO
						Block[] cBlocks = null;
						...
						PermissionStatus permissions = fsNamesys
					...
					case OP_DELETE: {
						...
						if (old != null && old.isUnderConstruction()) {
							INodeFileUnderConstruction cons = (INodeFileUnderConstruction) old;
							fsNamesys.leaseManager.removeLease(cons.clientName,
									path);
						}
						break;
					}
					...
					case OP_SET_PERMISSIONS: {
						...
						fsDir.unprotectedSetPermission(FSImage.readString(in),
								FsPermission.read(in));
						break;
					}
					case OP_SET_OWNER: {
						numOpSetOwner++;
						...
						fsDir.unprotectedSetOwner(FSImage.readString(in),
								FSImage.readString_EmptyAsNull(in), FSImage
										.readString_EmptyAsNull(in));
						break;
					}
					case OP_SET_QUOTA: {
						if (logVersion > -16) {
			...
			FSImage.LOG.info("Edits file " + edits.getName() + " of size "
					+ edits.length() + " edits # " + numEdits + " loaded in "
					+ (FSNamesystem.now() - startTime) / 1000 + " seconds.");
		}

		if (FSImage.LOG.isDebugEnabled()) {
			FSImage.LOG.debug("numOpAdd = " + numOpAdd + " numOpClose = "
					+ numOpClose + " numOpDelete = " + numOpDelete
					+ " numOpRename = " + numOpRename + " numOpSetRepl = "
					+ numOpSetRepl + " numOpMkDir = " + numOpMkDir
					+ " numOpSetPerm = " + numOpSetPerm + " numOpSetOwner = "
					+ numOpSetOwner + " numOpSetGenStamp = " + numOpSetGenStamp
	...
	private void printStatistics(boolean force) {
		...
		buf.append("Number of transactions: " + numTransactions
				+ " Total time for transactions(ms): " + totalTimeTransactions);
	...
	void logOpenFile(String path, INodeFileUnderConstruction newNode)
		...
		UTF8 nameReplicationPair[] = new UTF8[] { new UTF8(path),
	...
	void logMkDir(String path, INode newNode) {
		UTF8 info[] = new UTF8[] { new UTF8(path),
				FSEditLog.toLogLong(newNode.getModificationTime()) };
		logEdit(OP_MKDIR, new ArrayWritable(UTF8.class, info), newNode
				.getPermissionStatus());
	}
	...
	void logRename(String src, String dst, long timestamp) {
		UTF8 info[] = new UTF8[] { new UTF8(src), new UTF8(dst),
	...
	void logSetReplication(String src, short replication) {
		logEdit(OP_SET_REPLICATION, new UTF8(src), FSEditLog
	...
	void logDelete(String src, long timestamp) {
		UTF8 info[] = new UTF8[] { new UTF8(src),
				FSEditLog.toLogLong(timestamp) };
	...
	synchronized void rollEditLog() throws IOException {
		...
		if (existsNew()) {
			for (int idx = 0; idx < getNumStorageDirs(); idx++) {
				if (!getEditNewFile(idx).exists()) {
					throw new IOException(
							"Inconsistent existance of edits.new "
									+ getEditNewFile(idx));
				}
			}
		...
		for (int idx = 0; idx < getNumStorageDirs(); idx++) {
			try {
				EditLogOutputStream eStream = new EditLogOutputStream(
						getEditNewFile(idx));
				eStream.create();
				editStreams.add(eStream);
			} catch (IOException e) {
	...
	static private RSGroup[] readGroups(DataInputStream in) throws IOException {
		int numGroups = in.readInt();
		RSGroup[] groups = new RSGroup[numGroups];
		for(int i = 0; i < numGroups; i++){
			groups[i] = new RSGroup();
