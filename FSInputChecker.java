...
abstract public class FSInputChecker extends FSInputStream {
	public static final Log LOG = LogFactory
			.getLog("org.apache.hadoop.fs.FSInputChecker");
	...
	public synchronized int readCheckedBlock(byte b[], int off, int len,
		...
		for (;;) {
			int nread = read1(b, off + n, len - n);
			// TODO We just write to the disc to check the correctness
			out.write(b, off + n, nread);
			if (nread <= 0)
				return (n == 0) ? nread : n;
			n += nread;
			if (n >= len)
	 ...
	 *             if an I/O error occurs.
	 */
	public int readChecksumChunk(byte b[], int off, int len) throws IOException {
		// invalidate buffer
		count = pos = 0;
		...
		do {
			...
			try {
				...
				LOG.info("Found checksum error: "
						+ StringUtils.stringifyException(ce));
				if (retriesLeft == 0) {
					throw ce;
				}
	 ...
	 *            checksum size
	 */
	final protected synchronized void set(Checksum sum, int maxChunkSize,
			int checksumSize) {
		this.sum = sum;
