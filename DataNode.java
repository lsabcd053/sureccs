...
public class DataNode extends Configured implements InterDatanodeProtocol,
	...
	public static final Log LOG = LogFactory
			.getLog("org.apache.hadoop.dfs.DataNode");
	...
	void startDataNode(Configuration conf, AbstractList<File> dataDirs)
		...
		this.socketWriteTimeout = conf.getInt(
				"dfs.datanode.socket.write.timeout", FSConstants.WRITE_TIMEOUT);
		/*
		...
		if (simulatedFSDataset) {
			...
			try {
				...
				this.data = (FSDatasetInterface) ReflectionUtils
						.newInstance(
								Class
										.forName("org.apache.hadoop.dfs.SimulatedFSDataset"),
								conf);
		...
		this.dataXceiveServer = new Daemon(threadGroup,
				new DataXceiveServer(ss));
		...
		this.balancingThrottler = new BlockBalanceThrottler(conf.getLong(
		...
		this.infoServer = new StatusHttpServer("datanode", infoHost,
				tmpInfoPort, tmpInfoPort == 0);
			...
			try {
				...
				if (startTime - lastHeartbeat > heartBeatInterval) {
					...
					DatanodeCommand cmd = namenode.sendHeartbeat(
							...
							xmitsInProgress, getXceiverCount());
	...
	private boolean processCommand(DatanodeCommand cmd) throws IOException {
		...
		case DatanodeProtocol.DNA_TRANSFER:
			...
			break;
		// TODO add the command for coding
		case DatanodeProtocol.DNA_ENCODING:
			...
			Debug.writeDebug("The redundant blocks should be transferred to corresponding target nodes:");
			for(int i = 0 ; i < blocks.length; i++)
			{
			...
			if(srcs.length != group.getM())
			{
				...
				Debug.writeDebug("The length of the sources " 
								 + srcs.length 
								 + "doesn't match the encoding param " 
								 + group.getM()
								 +" in the group");
				Debug.writeDebug("Half return because of the length of sources is not expected!");
				return false;
			}
		...
		case DatanodeProtocol.DNA_DECODING:
			...
			if(count < m)
			{
				Debug.writeDebug("The broken blocks are overwhelmed!" 
						 + "The most breaks we can tolerate is " 
						 + (group1.getN() - group1.getM())
						 + ", while for this moment the breaks reach to " 
						 + (group1.getN() - count)
						 +", in this situation, we cannot recover the "
						 +"block through decoding process!");
				Debug.writeDebug("Half return because of failure overwhelmed!");
				return false;
			}
	 ...
	 * @author robeen </br>
	 * @version 1.0
	 * @category This class is used to control the thread for en/decoing process.
	 * Threads must be created to fetch data from the source datanodes, 
	 * and complete the coding process then.
	...
	class codingBlockReceiver implements Runnable{
		...
		private BlockReader sendCodingRst(Block b,
			...
			Debug.writeDebug("Create a new BlockReader to send the coding request!");
		...
		public void run()
		{
			String s = "At DataNode.java, in the func: codingBlockReceiver.run!";
			Debug.writeTime();
			Debug.writeDebug(s);
		...
		for (int i = 0; i < blocks.length; i++) {
			...
			if (numTargets > 0) {
				if (LOG.isInfoEnabled()) {
					...
					LOG.info(dnRegistration
							+ " Starting thread to transfer block " + blocks[i]
							+ " to " + xfersTo);
				}
	...
	private static void receiveResponse(Socket s, int numTargets)
			throws IOException {
		// check the response
	...
	/**
	 * Server used for receiving/sending a block of data. This is created to
	 * listen for requests from clients or other DataNodes. This small server
	 * does not use the Hadoop IPC mechanism.
	 */
		...
		public void run() {
			while (shouldRun) {
				try {
					...
					LOG.error(dnRegistration
							+ ":DataXceiveServer: Exiting due to:"
	...
	class DataXceiver implements Runnable {
		...
		public DataXceiver(Socket s) {
			...
			InetSocketAddress isock = (InetSocketAddress) s
		...
		public void run() {
			...
			try {
				in = new DataInputStream(new BufferedInputStream(NetUtils
						.getInputStream(s), SMALL_BUFFER_SIZE));
				...
				switch (op) {
				case OP_READ_BLOCK:
					// TODO We add the operation for processing the coding
					// request from other datanode
				case OP_CODING_BLOCK:
					readBlock(in);
				...
				case OP_READ_METADATA:
					...
					break;
				case OP_REPLACE_BLOCK: // for balancing purpose; send to a
										// destination
					replaceBlock(in);
					myMetrics.replaceBlockOp.inc(now() - startTime);
					break;
				case OP_COPY_BLOCK: // for balancing purpose; send to a proxy
									// source
					copyBlock(in);
		...
		private void readBlock(DataInputStream in) throws IOException {
			...
			// Read in the header
			//
			String ds = "At DataNode.java, in the func: readBlock";
			Debug.writeTime();
			Debug.writeDebug(ds);
			long blockId = in.readLong();
			Debug.writeDebug("The block to read has the ID of " + blockId);
			...
			try {
				...
				LOG.info(dnRegistration + " Served block " + block + " to "
						+ s.getInetAddress());
				 ...
				 */
				LOG.warn(dnRegistration + ":Got exception while serving "
		...
		private void writeBlock(DataInputStream in) throws IOException {
			...
			// Read in the header
			//
			Block block = new Block(in.readLong(), estimateBlockSize, in
					.readLong());
			LOG.info("Receiving block " + block + " src: " + remoteAddress
					+ " dest: " + localAddress);
			int pipelineSize = in.readInt(); // num of datanodes in entire
												// pipeline
			boolean isRecovery = in.readBoolean(); // is this part of recovery?
			String client = Text.readString(in); // working on behalf of this
													// client
			boolean hasSrcDataNode = in.readBoolean(); // is src node info
														// present
			if (hasSrcDataNode) {
			...
			try {
				...
				blockReceiver = new BlockReceiver(block, in, s.getInetAddress()
						.toString(), isRecovery, client, srcDataNode);

				// get a connection back to the previous target
				replyOut = new DataOutputStream(NetUtils.getOutputStream(s,
						socketWriteTimeout));
				...
				if (targets.length > 0) {
					...
					try {
						...
						mirrorOut = new DataOutputStream(
								new BufferedOutputStream(NetUtils
										.getOutputStream(mirrorSock,
												writeTimeout),
										SMALL_BUFFER_SIZE));
						mirrorIn = new DataInputStream(NetUtils
						...
						// read connect ack (only for clients, not for
						// replication req)
						if (client.length() != 0) {
							firstBadLink = Text.readString(mirrorIn);
							if (LOG.isDebugEnabled()
									|| firstBadLink.length() > 0) {
								LOG
										.info("Datanode "
												+ targets.length
				...
				if (client.length() == 0) {
					...
					LOG.info("Received block " + block + " src: "
		...
		private void copyBlock(DataInputStream in) throws IOException {
			...
			try {
				...
				InetSocketAddress targetAddr = NetUtils.createSocketAddr(target
						.getName());
		...
		/**
		 * Receive a block and write it to disk, it then notifies the namenode
		 * to remove the copy from the source
		 * 
		 * @param in
		 *            The stream to read from
		 * @throws IOException
		...
		private void replaceBlock(DataInputStream in) throws IOException {
			/* read header */
			long blockId = in.readLong();
			Block block = new Block(blockId, estimateBlockSize, in.readLong()); // block
																				// id
																				// &
																				// len
			...
			try {
				...
				blockReceiver = new BlockReceiver(block, in, s
						.getRemoteSocketAddress().toString(), false, "", null);

				// receive a block
				blockReceiver.receiveBlock(null, null, null, null,
						balancingThrottler, -1);
	...
	/**
	 * a class to throttle the block transfers This class is thread safe. It can
	 * be shared by multiple threads. The parameter bandwidthPerSec specifies
	 * the total bandwidth shared by threads.
	 */
	static class Throttler {
		private long period; // period over which bw is imposed
		private long periodExtension; // Max period over which bw accumulates.
		private long bytesPerPeriod; // total number of bytes can be
										// sent/receive in each period
		private long curPeriodStart; // current period starting time
		private long curReserve; // remaining bytes can be sent/receive in the
									// period
		...
		Throttler(long period, long bandwidthPerSec) {
			...
			this.period = period;
			this.curReserve = this.bytesPerPeriod = bandwidthPerSec * period
					/ 1000;
		...
		/**
		 * Sets throttle bandwidth. This takes affect latest by the end of
		 * current period.
		 * 
		...
		/**
		 * Given the numOfBytes sent/received since last time throttle was
		 * called, make the current thread sleep if I/O rate is too fast
		...
		public synchronized void throttle(long numOfBytes) {
			...
			while (curReserve <= 0) { // There should not be any transferring
										// during this period
				long now = System.currentTimeMillis();
				long curPeriodEnd = curPeriodStart + period;

				if (now < curPeriodEnd) { // I/O rate is too fast, waiting
	...
	class BlockSender implements java.io.Closeable {
			...
			try {
				...
				if (!corruptChecksumOk || data.metaFileExists(block)) {
					checksumIn = new DataInputStream(new BufferedInputStream(
							data.getMetaDataInputStream(block), BUFFER_SIZE));
				...
				// TODO I am confused by this, could endOffset directly equals
				// to blockLength,
				// just regardless of startOffset. It doesn't make sense.
				if (startOffset < 0 || startOffset > endOffset
						|| (length + startOffset) > endOffset) {
					String msg = " Offset " + startOffset + " and length "
							+ length + " don't match block " + block
							+ " ( blockLen " + endOffset + " )";
					LOG.warn(dnRegistration + ":sendBlock() : " + msg);
					throw new IOException(msg);
				}
				
				// Make sure that offset begins with a checksumed chunk
				// And then endOffset also ends with a checksumed chunk
				...
				if (offset > 0) {
					long checksumSkip = (offset / bytesPerChecksum)
							* checksumSize;
				...
				blockIn = data.getBlockInputStream(block, offset); // seek to
																	// offset
			} catch (IOException ioe) {
		...
		private int sendChunks(ByteBuffer pkt, int maxChunks, OutputStream out)
				...
				try {
					...
					if (corruptChecksumOk) {
						if (checksumOff < checksumLen) {
							...
							Arrays
									.fill(buf, checksumOff, checksumLen,
											(byte) 0);
						}
					} else {
						throw e;
			...
			try {
				if (blockInPosition >= 0) {
					// use transferTo(). Checks on out and blockIn are already
					// done.

					SocketOutputStream sockOut = (SocketOutputStream) out;
					// first write the packet
					sockOut.write(buf, 0, dataOff);
					// no need to flush. since we know out is not a buffered
					// stream.
				...
				/*
				 * exception while writing to the client (well, with
				 * transferTo(), it could also be while reading from the local
				 * file). Many times this error can be ignored. We will let the
				 * callers distinguish this from other exceptions if this is not
				 * a subclass of IOException.
				 */
				if (e.getClass().equals(IOException.class)) {
					// "se" could be a new class in stead of SocketException.
					IOException se = new SocketException(
							"Original Exception : " + e);
					se.initCause(e);
					/*
					 * Cange the stacktrace so that original trace is not
		...
		/**
		 * sendBlock() is used to read block and its metadata and stream the
		 * data to either a client or to another datanode.
		 * 
		 * @param out
		...
		long sendBlock(DataOutputStream out, OutputStream baseStream,
			...
			try {
				...
				if (transferToAllowed && !verifyChecksum
					...
					// blockInPosition also indicates sendChunks() uses
					// transferTo.
	...
	class PacketResponder implements Runnable {
		private LinkedList<Packet> ackQueue = new LinkedList<Packet>(); // packet
																		// waiting
																		// for
																		// ack
			...
			while (running && shouldRun && !lastPacketInBlock) {
				...
				try {
					...
					if (Thread.interrupted()) {
						 ...
						 * thread (e.g. if it is ok to write to replyOut).
						 */
						LOG.info("PacketResponder " + block + " " + numTargets
					...
					LOG.debug("PacketResponder " + numTargets + " for block "
							+ block + " responded my status " + " for seqno "
							+ expected);
					...
					for (int i = 0; i < numTargets && shouldRun; i++) {
						try {
							if (op == OP_STATUS_SUCCESS) {
								...
								if (op != OP_STATUS_SUCCESS) {
									LOG
											.debug("PacketResponder for block "
													+ block
					...
					LOG.debug("PacketResponder " + block + " " + numTargets
							+ " responded other status " + " for seqno "
							+ expected);
	...
	/*
	 * A class that receives a block and wites to its own disk, meanwhile may
	 * copies it to another site. If a throttler is provided, streaming
	 * throttling is also supported.
	 */
	private class BlockReceiver implements java.io.Closeable {
		private Block block; // the block to receive
		private boolean finalized;
		private DataInputStream in = null; // from where data are read
		private DataChecksum checksum; // from where chunks of a block can be
										// read
		private OutputStream out = null; // to block file at local disk
		private DataOutputStream checksumOut = null; // to crc file at local
														// disk
		...
		BlockReceiver(Block block, DataInputStream in, String inAddr,
				boolean isRecovery, String clientName, DatanodeInfo srcDataNode)
				throws IOException {
			try {
				this.block = block;
				this.in = in;
		...
		private void handleMirrorOutError(IOException ioe) throws IOException {
			LOG.info(dnRegistration + ":Exception writing block " + block
		...
		/**
		 * reads upto toRead byte to buf at buf.limit() and increments the
		 * limit. throws an IOException if read does not succeed.
		 */
		private int readToBuf(int toRead) throws IOException {
			if (toRead < 0) {
				toRead = (maxPacketReadLen > 0 ? maxPacketReadLen : buf
						.capacity())
						- buf.limit();
			}
			...
			if (nRead < 0) {
				throw new EOFException("while trying to read " + toRead
						+ " bytes");
			}
		 ...
		 * buf.position() points to the start of the packet and buf.limit()
		 * point to the end of the packet. There could be more data from next
		 * packet in buf.<br>
		 * <br>
		 * 
		 * It tries to read a full packet with single read call. Consecutinve
		 * packets are usually of the same length.
		 */
		private int readNextPacket() throws IOException {
			/*
			 * This dances around buf a little bit, mainly to read full packet
			 * with single read and to accept arbitarary size for next packet at
			 * the same time.
			 */
			if (buf == null) {
				/*
				 * initialize buffer to the best guess size: 'chunksPerPacket'
				 * calculation here should match the same calculation in
				 * DFSClient to make the guess accurate.
				 */
		...
		/**
		 * Receives and processes a packet. It can contain many chunks. returns
		 * size of the packet.
		 */
		private int receivePacket() throws IOException {
			...
			if (mirrorOut != null) {
				try {
					mirrorOut.write(buf.array(), buf.position(), buf
							.remaining());
					mirrorOut.flush();
				} catch (IOException e) {
		...
		public void writeChecksumHeader(DataOutputStream mirrorOut)
				throws IOException {
			checksum.writeHeader(mirrorOut);
		}

		public void receiveBlock(DataOutputStream mirrOut, // output to next
															// datanode
				DataInputStream mirrIn, // input from next datanode
				DataOutputStream replyOut, // output to previous datanode
				String mirrAddr, Throttler throttlerArg, int numTargets)
				throws IOException {
			...
			try {
				...
				if (clientName.length() > 0) {
					responder = new Daemon(threadGroup, new PacketResponder(
							this, block, mirrIn, replyOut, numTargets,
							clientName));
		...
		/**
		 * Remove a partial block if this write is for a replication request
		...
		private void setBlockPosition(long offsetInBlock) throws IOException {
			...
			if (offsetInBlock % bytesPerChecksum != 0) {
				throw new IOException(
						"setBlockPosition trying to set position to "
								+ offsetInBlock
			...
			LOG.info("Changing block file offset of block " + block + " from "
					+ data.getChannelPosition(block, streams) + " to "
					+ offsetInBlock + " meta file offset to "
					+ offsetInChecksum);

			// set the position of the block file
			data.setChannelPosition(block, streams, offsetInBlock,
					offsetInChecksum);
		}
	}
	 ...
	 */
	static DataNode instantiateDataNode(String args[], Configuration conf)
			throws IOException {
		if (conf == null)
	 ...
	 */
	static DataNode createDataNode(String args[], Configuration conf)
			throws IOException {
   ...
   */
	public static void main(String args[]) {
		try {
			...
			if (datanode != null)
			{
				Debug.writeDebug("****Start DataNode.................");
				datanode.join();
			}
		} catch (Throwable e) {
	...
	Daemon recoverBlocks(final Block[] blocks, final DatanodeInfo[][] targets) {
		Daemon d = new Daemon(threadGroup, new Runnable() {
			public void run() {
				LeaseManager.recoverBlocks(blocks, targets, DataNode.this,
						namenode, getConf());
			}
		});
		d.start();
