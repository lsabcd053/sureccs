...
class DFSClient implements FSConstants {
	 ...
	 */
	private TreeMap<String, OutputStream> pendingCreates = new TreeMap<String, OutputStream>();
	...
	private static ClientProtocol createRPCNamenode(
		...
		return (ClientProtocol) RPC.getProxy(ClientProtocol.class,
				ClientProtocol.versionID, nameNodeAddr, ugi, conf, NetUtils
	...
	static ClientDatanodeProtocol createClientDatanodeProtocolProxy(
		...
		if (ClientDatanodeProtocol.LOG.isDebugEnabled()) {
			ClientDatanodeProtocol.LOG.info("ClientDatanodeProtocol addr="
					+ addr);
		}
		return (ClientDatanodeProtocol) RPC.getProxy(
	...
	public DFSClient(InetSocketAddress nameNodeAddr, Configuration conf)
			throws IOException {
		this(nameNodeAddr, conf, FileSystem
				.getStatistics(DistributedFileSystem.class));
	}
				...
				while (!pendingCreates.isEmpty()) {
					...
					if (out != null) {
						try {
							...
							System.err
									.println("Exception closing file " + name);
							ie.printStackTrace();
						}
					}
				}
	...
	public BlockLocation[] getBlockLocations(String src, long start, long length)
		...
		for (LocatedBlock blk : blocks.getLocatedBlocks()) {
			...
			blkLocations[idx] = new BlockLocation(names, hosts, blk
					.getStartOffset(), blk.getBlockSize());
			idx++;
		}
		return blkLocations;
	}
	...
	public OutputStream create(String src, boolean overwrite,
			...
			throws IOException {
		return create(src, overwrite, replication, blockSize, progress, conf
   ...
   */
	 ...
	 *            The string representation of the path to the directory
	 * @param quota
	 *            The limit of the number of names in the tree rooted at the
	 *            directory
	 * @throws FileNotFoundException
	...
	void setQuota(String src, long quota) throws IOException {
		try {
			...
			throw re.unwrapRemoteException(AccessControlException.class,
					FileNotFoundException.class, QuotaExceededException.class);
		}
	}
	...
	static class BlockReader extends FSInputChecker {
		...
		public synchronized int read(byte[] buf, int off, int len)
			...
			if (gotEOS && !eosBefore && nRead >= 0 && needChecksum()) {
				if (sentChecksumOk) {
					...
					LOG
							.info(StringUtils
									.stringifyException(new IOException(
		...
		private void adjustChecksumBytes(int dataLen) {
			...
			if (checksumBytes == null
					|| requiredSize > checksumBytes.capacity()) {
		...
		protected synchronized int readChunk(long pos, byte[] buf, int offset,
			...
			if (gotEOS) {
				if (startOffset < 0) {
					...
					throw new IOException(
							"BlockRead: already got EOS or an error");
				}
				startOffset = -1;
				return -1;
			}
			...
			if (dataLeft <= 0) {
				...
				if (dataLen < 0
					...
					throw new IOException("BlockReader: error in packet header"
							+ "(chunkOffset : " + chunkOffset + ", dataLen : "
							+ dataLen + ", seqno : " + seqno + " (last: "
							+ lastSeqNo + "))");
				}
		...
		public BlockReader(String file, long blockId, DataInputStream in,
				DataChecksum checksum, boolean verifyChecksum,
				long startOffset, long firstChunkOffset, Socket dnSock) {
			super(new Path("/blk_" + blockId + ":of:" + file)/*
															 * too non
															 * path-like?
															 */, 1,
					verifyChecksum, checksum.getChecksumSize() > 0 ? checksum
							: null, checksum.getBytesPerChecksum(), checksum
							.getChecksumSize());
		...
		static BlockReader newBlockReader(Socket sock, String file,
				long blockId, long genStamp, long startOffset, long len,
				int bufferSize) throws IOException {
		...
		static BlockReader newBlockReader(Socket sock, String file,
			...
			if (firstChunkOffset < 0
					...
					|| firstChunkOffset >= (startOffset + checksum
							.getBytesPerChecksum())) {
				throw new IOException(
						"BlockReader: error in first chunk offset ("
		...
		private void checksumOk(Socket sock) {
			try {
				OutputStream out = NetUtils
						.getOutputStream(sock, WRITE_TIMEOUT);
				...
				LOG.debug("Could not write to datanode "
	...
	class DFSInputStream extends FSInputStream {
		...
		synchronized void openInfo() throws IOException {
			...
			if (locatedBlocks != null) {
				Iterator<LocatedBlock> oldIter = locatedBlocks
						.getLocatedBlocks().iterator();
				Iterator<LocatedBlock> newIter = newInfo.getLocatedBlocks()
						.iterator();
				while (oldIter.hasNext() && newIter.hasNext()) {
					if (!oldIter.next().getBlock().equals(
							newIter.next().getBlock())) {
						throw new IOException("Blocklist for " + src
								+ " has changed!");
					}
				}
			}
		...
		private LocatedBlock getBlockAt(long offset) throws IOException {
			...
			if (targetBlockIdx < 0) { // block is not cached
				...
				assert (newBlocks != null) : "Could not find target position "
						+ offset;
				locatedBlocks.insertRange(targetBlockIdx, newBlocks
						.getLocatedBlocks());
			}
		...
		private synchronized List<LocatedBlock> getBlockRange(long offset,
			...
			while (remaining > 0) {
				...
				if (blk == null || curOff < blk.getStartOffset()) {
					...
					locatedBlocks.insertRange(blockIdx, newBlocks
							.getLocatedBlocks());
					continue;
				}
				assert curOff >= blk.getStartOffset() : "Block not found";
				blockRange.add(blk);
				long bytesRead = blk.getStartOffset() + blk.getBlockSize()
						- curOff;
				remaining -= bytesRead;
				curOff += bytesRead;
				blockIdx++;
		...
		private synchronized int readBuffer(byte buf[], int off, int len)
			...
			while (true) {
				...
				try {
					...
					LOG.warn("Found Checksum error for " + currentBlock
							+ " from " + currentNode.getName() + " at "
							+ ce.getPos());
			...
			while (true) {
				...
				try {
					...
					InetSocketAddress targetAddr = NetUtils
							.createSocketAddr(chosenNode.getName());
		...
		private void fetchBlockByteRange(LocatedBlock block, long start,
			...
			while (dn == null && numAttempts-- > 0) {
				...
				try {
					...
					reader = BlockReader.newBlockReader(dn, src, block
							.getBlock().getBlockId(), block.getBlock()
							.getGenerationStamp(), start, len, buffersize,
							verifyChecksum);
					int nread = reader.readAll(buf, offset, len);
					if (nread != len) {
						throw new IOException(
								"truncated return from reader.read(): "
		...
		public int read(long position, byte[] buffer, int offset, int length)
			...
			for (LocatedBlock blk : blockRange) {
				...
				long bytesToRead = Math.min(remaining, blk.getBlockSize()
						- targetStart);
				fetchBlockByteRange(blk, targetStart, targetStart + bytesToRead
						- 1, buffer, offset);
	...
	class DFSOutputStream extends FSOutputSummer implements Syncable {
		...
		private boolean isClient = true; // Default is from client

		// While we have different situation which is coming from the datanode
		// to process the en/decoding process.
		...
		private class Packet {
			...
			ByteBuffer getBuffer() {
				...
				if (checksumPos != dataStart) {
					 ...
					 * the last packet.
					 */
					System.arraycopy(buf, checksumStart, buf, dataStart
					...
					synchronized (dataQueue) {
						...
						try {
							...
							if (blockStream == null) {
								...
								// TODO default is called by the client
								// else call REPERA's getBlockOutput Stream
								if(isClient)
									nodes = nextBlockOutputStream(src);
								else {
									if( nodes!= null )
										getBlockOutputStream(src, nodes);
								}
				...
				while (!closed && clientRunning && !lastPacketInBlock) {
					...
					try {
						...
						if (seqno == -1) {
							...
							if (one.seqno != seqno) {
								throw new IOException(
										"Responseprocessor: Expecting seqno "
												+ " for block " + block
		...
		private boolean processDatanodeError() {
			...
			String msg = "Error Recovery for block " + block + " bad datanode["
					+ errorIndex + "]";
			if (nodes != null) {
			...
			while (!success && clientRunning) {
				...
				if (nodes.length <= 1) {
					lastException = new IOException("All datanodes " + pipeline
							+ " are bad. Aborting...");
					closed = true;
					streamer.close();
					return false;
				...
				try {
					...
					// avoid deadlock.
					primary = createClientDatanodeProtocolProxy(Collections
							.min(Arrays.asList(newnodes)), conf);
					newBlock = primary.recoverBlock(block, newnodes);
				} catch (IOException e) {
					recoveryErrorCount++;
					if (recoveryErrorCount > maxRecoveryErrorCount) {
						String emsg = "Error Recovery for block " + block
								+ " failed "
		...
		public DFSOutputStream(String src, FsPermission masked,
			...
			this.isClient = true;
			if (progress != null) {
				LOG.debug("Set non-null progress callback on DFSOutputStream "
						+ src);
			}
		...
		public boolean getBlockOutputStream(String client, DatanodeInfo[] nodes)
				throws IOException {
			String s = "At DFSClient.java, in the func: getBlockOutputStream";
			Debug.writeTime();
			Debug.writeDebug(s);
			boolean retry = false;
			...
			if(!success) {
				Debug.writeDebug("Creating block output stream failed!");
		...
		private boolean createBlockOutputStream(DatanodeInfo[] nodes,
			...
			try {
				...
				InetSocketAddress target = NetUtils.createSocketAddr(nodes[0]
						.getName());
				s = socketFactory.createSocket();
				...
				blockReplyStream = new DataInputStream(NetUtils
						.getInputStream(s));
					...
					try {
						...
						if (--retries == 0
							...
							if (System.currentTimeMillis() - localstart > 5000) {
								LOG
										.info("Waiting for replication for "
												+ (System.currentTimeMillis() - localstart)
												/ 1000 + " seconds");
							}
							try {
								LOG.warn("NotReplicatedYetException sleeping "
		...
		protected synchronized void writeChunk(byte[] b, int offset, int len,
			...
			if (checksum.length != this.checksum.getChecksumSize()) {
				throw new IOException(
						"writeChunk() checksum size is supposed to be "
			...
			synchronized (dataQueue) {
				...
				while (!closed
						&& dataQueue.size() + ackQueue.size() > maxPackets) {
					try {
						dataQueue.wait();
					} catch (InterruptedException e) {
					}
		...
		synchronized void setChunksPerPacket(int value) {
			...
			packetSize = DataNode.PKT_HEADER_LEN
					+ SIZE_OF_INTEGER
					+ (checksum.getBytesPerChecksum() + checksum
							.getChecksumSize()) * chunksPerPacket;
		}
