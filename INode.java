...
abstract class INode implements Comparable<byte[]> {
	...
	protected FsPermission getFsPermission() {
		return new FsPermission((short) PermissionStatusFormat.MODE
				.retrieve(permission));
	}
	...
	protected void setPermission(FsPermission permission) {
		updatePermissionStatus(PermissionStatusFormat.MODE, permission
				.toShort());
	}
	...
	/**
	 * Get the total number of names in the tree rooted at this inode including
	 * the root
	 * 
...
class INodeDirectory extends INode {
   ...
   */
	 ...
	 * @return null if the child with this name already exists; inserted INode,
	 *         otherwise
	 */
		...
		if (inheritPermission) {
			...
			if (!p.getUserAction().implies(FsAction.WRITE_EXECUTE)) {
				p = new FsPermission(p.getUserAction().or(
						FsAction.WRITE_EXECUTE), p.getGroupAction(), p
						.getOtherAction());
			}
...
class INodeDirectoryWithQuota extends INodeDirectory {
	 ...
	 * 
	 * @param quota
	 *            Quota to be assigned to this inode
	 * @param other
	...
	INodeDirectoryWithQuota(PermissionStatus permissions,
			long modificationTime, long quota) {
	 ...
	 *            Quota to be set
	 * @throws QuotaExceededException
	 *             if the given quota is less than the size of the tree
	 */
	 ...
	 * @throws QuotaExceededException
	 *             if the given count is greater than quota
	 */
...
class INodeFile extends INode {
	...
	// TODO Add the group List in InodeFile
	// The group will not currently support the FSImage,
	// Obviously, we will get an updated modification to 
	// support that operation.
	protected RSGroup groups[] = null;
	protected BlockInfo codingBlocks[] = null;
	protected int RSn;
	protected int RSm;
	...
	INodeFile(PermissionStatus permissions, int nrBlocks, short replication,
			long modificationTime, long preferredBlockSize) {
	...
	protected INodeFile(PermissionStatus permissions, BlockInfo[] blklist,
			short replication, long modificationTime, long preferredBlockSize) {
		super(permissions, modificationTime);
	...
	// INodeFile and INodeFileUnderConstruction
	protected INodeFile(PermissionStatus permissions, BlockInfo[] blklist,
			RSGroup[] grpList, BlockInfo[] codedBlkList, short replication,
			long modificationTime, long preferredBlockSize, int n, int m) {
		super(permissions, modificationTime);
		this.blockReplication = replication;
		this.preferredBlockSize = preferredBlockSize;
	...
	*/
	 ...
	 * @input RSGroup newGroup:��ǰҪ��ӵ�group
	 * @return void
	 */
	public void addGroup(RSGroup newGroup) throws IOException {
		...
		if (this.groups == null) {
			...
			RSGroup[] newlist = new RSGroup[size + 1];
			for (int i = 0; i < size; i++) {
				newlist[i] = this.groups[i];
			}
			newlist[size] = newGroup;
			this.groups = newlist;
		}
	}

	public RSGroup getLastGroup() {
	...
	/*
	 * ���������ӵ�����group��
	 * Function Name: addCodingGroup
	 * @input BlockInfo newBlock:��ǰҪ��ӵ������
	 * @return void
	 */
	void addCodingBlock(BlockInfo newblock) throws IOException {
		...
		if (this.codingBlocks == null) {
			...
			int size = this.codingBlocks.length;
			BlockInfo[] newlist = new BlockInfo[size + 1];
			for (int i = 0; i < size; i++) {
				newlist[i] = this.codingBlocks[i];
			}
			newlist[size] = newblock;
			this.codingBlocks = newlist;
		}
		RSGroup lastGroup = this.getLastGroup();
		lastGroup.addBlock(newblock);
	}

	BlockInfo[] getCodingBlocks() {
	...
	RSGroup getGroupfromBlock(BlockInfo block) throws IOException {		
		...
		int i = 0;
		for (; i < size; i++) {
			if (-1 != this.groups[i].getGroupfromBlock(block))
				break;
		}
		if (i == size){
	...
	void addBlock(BlockInfo newblock, int n, int m) throws IOException {
		...
		RSGroup lastGroup = this.groups[grpSize - 1];
		lastGroup.addBlock(newblock);
		// TODO
	}

	// TODO New code added, to get the current Size of Blocks in INodeFile
	int getBlockSize() {
		if (this.blocks == null)
			return 0;
		else
	...
	INodeFileUnderConstruction toINodeFileUnderConstruction(String clientName,
			String clientMachine, DatanodeDescriptor clientNode)
			throws IOException {
		if (isUnderConstruction()) {
			return (INodeFileUnderConstruction) this;
		}
		// TODO change to support groups and codingBlocks
		return new INodeFileUnderConstruction(name, blockReplication,
				modificationTime, preferredBlockSize, blocks, groups, 
				codingBlocks,
				getPermissionStatus(), clientName, clientMachine, clientNode, RSn, RSm);
		// TODO
		
		// TODO Original 
		//return new INodeFileUnderConstruction(name, blockReplication,
				//modificationTime, preferredBlockSize, blocks, 
...
class INodeFileUnderConstruction extends INodeFile {
	...
	INodeFile convertToInodeFile() {
		...
		//INodeFile obj = new INodeFile(getPermissionStatus(), getBlocks(),
				//getReplication(),getModificationTime(),getPreferredBlockSize());
		//TODO change to support groups and codingBlocks
		INodeFile obj = new INodeFile(getPermissionStatus(), getBlocks(),
				getGroups(),getCodingBlocks(),getReplication(),
				getModificationTime(),getPreferredBlockSize(), RSn, RSm);
		//TODO
		return obj;
