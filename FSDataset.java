	...
	/**************************************************
	 * FSDataset manages a set of data blocks. Each block has a unique name and
	 * an extent on disk.
	 * 
	 ***************************************************/
     ...
     */
			...
			private File addBlock(Block b, File src, boolean createOk,
				...
				if (lastChildIdx >= 0 && children != null) {
					...
					for (int i = 0; i < children.length; i++) {
						...
						File file = children[idx].addBlock(b, src, false,
								resetIdx);
						if (file != null) {
							lastChildIdx = idx;
							return file;
			...
			/**
			 * Populate the given blockSet with any child blocks found at this
			 * node.
			 */
				...
				for (int i = 0; i < blockFiles.length; i++) {
					if (Block.isBlockFilename(blockFiles[i])) {
						long genStamp = getGenerationStampFromFile(blockFiles,
								blockFiles[i]);
						blockSet.add(new Block(blockFiles[i], blockFiles[i]
								.length(), genStamp));
					}
				}
			}
				...
				for (int i = 0; i < blockFiles.length; i++) {
					if (Block.isBlockFilename(blockFiles[i])) {
						...
						volumeMap.put(new Block(blockFiles[i], blockFiles[i]
								.length(), genStamp), new DatanodeBlockInfo(
								volume, blockFiles[i]));
					}
				}
			}
			...
			void clearPath(File f) {
				...
				if (dir.startsWith(root)) {
					String[] dirNames = dir.substring(root.length()).split(
			...
			/*
			 * dirNames is an array of string integers derived from usual
			 * directory structure data/subdirN/subdirXY/subdirM ... If dirName
			 * array is non-null, we only check the child at the
			 * children[dirNames[idx]]. This avoids iterating over children in
			 * common case. If directory structure changes in later versions, we
			...
			private boolean clearPath(File f, String[] dirNames, int idx) {
				...
				if (dirNames != null) {
					...
					return (childIdx >= 0 && childIdx < children.length) ? children[childIdx]
							.clearPath(f, dirNames, idx + 1)
							: false;
				}
		...
		class FSVolume {
			...
			private File createTmpFile(Block b, File f) throws IOException {
				...
				try {
					...
					throw (IOException) new IOException(DISK_ERROR + f)
							.initCause(ioe);
				}
				if (!fileCreated) {
					throw new IOException(
			...
			private void recoverDetachedBlocks(File dataDir, File dir)
				...
				for (int i = 0; i < contents.length; i++) {
					if (!contents[i].isFile()) {
						throw new IOException("Found " + contents[i] + " in "
								+ dir + " but it is not a file.");
					}
					...
					if (!blk.exists()) {
						if (!contents[i].renameTo(blk)) {
							throw new IOException(
									"Unable to recover detached file "
											+ contents[i]);
						}
						continue;
					}
					if (!contents[i].delete()) {
						throw new IOException(
								"Unable to cleanup detached file "
										+ contents[i]);
					}
				}
			}
		...
		static class FSVolumeSet {
			...
			synchronized void getVolumeMap(
					HashMap<Block, DatanodeBlockInfo> volumeMap) {
				for (int idx = 0; idx < volumes.length; idx++) {
		...
		static File getMetaFile(File f, Block b) {
			return new File(getMetaFileName(f.getAbsolutePath(), b
					.getGenerationStamp()));
		}
		...
		private static File findMetaFile(final File blockFile)
				throws IOException {
			...
			if (matches == null || matches.length == 0) {
				throw new IOException("Meta file not found, blockFile="
						+ blockFile);
		...
		public synchronized Block getStoredBlock(long blkid) throws IOException {
			...
			return new Block(blkid, blockfile.length(), parseGenerationStamp(
					blockfile, metafile));
		}
		...
		//TODO
		/**
		 * Get the blk-referred volume
		 */
		public FSVolume getVolumeFromBlock(Block blk)
		{
			DatanodeBlockInfo info = volumeMap.get(blk);
		...
		protected synchronized File getBlockFile(Block b) throws IOException {
			...
			if (f == null) {
				if (InterDatanodeProtocol.LOG.isDebugEnabled()) {
					InterDatanodeProtocol.LOG.debug("b=" + b + ", volumeMap="
							+ volumeMap);
				}
		...
		private BlockWriteStreams createBlockWriteStreams(File f, File metafile)
				throws IOException {
			return new BlockWriteStreams(new FileOutputStream(
					new RandomAccessFile(f, "rw").getFD()),
		 ...
		 * snapshot. This ensures that modifying this block does not modify data
		 * in any existing snapshots.
		 * 
		 * @param b
		 *            Block
		 * @param numLinks
		...
		/** {@inheritDoc} */
		public void updateBlock(Block oldblock, Block newblock)
				throws IOException {
		...
		/**
		 * Try to update an old block to a new block. If there are ongoing
		 * create threads running for the old block, the threads will be
		 * returned without updating the block.
		 * 
		 * @return ongoing create threads if there is any. Otherwise, return
		 *         null.
		 */
		private synchronized List<Thread> tryUpdateBlock(Block oldblock,
			...
			if (activefile != null && !activefile.threads.isEmpty()) {
				// remove dead threads
				for (Iterator<Thread> i = activefile.threads.iterator(); i
						.hasNext();) {
			...
			File tmpMetaFile = new File(oldMetaFile.getParent(), oldMetaFile
					.getName()
					+ "_tmp" + newblock.getGenerationStamp());
			...
			if (oldgs > newblock.generationStamp) {
				throw new IOException("Cannot update block (id="
						+ newblock.blkid + ") generation stamp from " + oldgs
			...
			if (newblock.len > oldblock.len) {
				throw new IOException("Cannot update block file (=" + blockFile
						+ ") length from " + oldblock.len + " to "
						+ newblock.len);
			}
			if (newblock.len < oldblock.len) {
				truncateBlock(blockFile, tmpMetaFile, oldblock.len,
						newblock.len);
			}

			// rename the tmp file to the new meta file (with new generation
			// stamp)
		...
		static private void truncateBlock(File blockFile, File metaFile,
			...
			if (newlen > oldlen) {
				throw new IOException(
						"Cannout truncate block to from oldlen (=" + oldlen
								+ ") to newlen (=" + newlen + ")");
			}

			DataChecksum dcs = BlockMetadataHeader.readHeader(metaFile)
					.getChecksum();
			int checksumsize = dcs.getChecksumSize();
			int bpc = dcs.getBytesPerChecksum();
			long n = (newlen - 1) / bpc + 1;
			long newmetalen = BlockMetadataHeader.getHeaderSize() + n
					* checksumsize;
		 ...
		 * @param ioe
		 *            an I/O exception
		 * @return cause if the I/O exception is caused by a possible disk
		 ...
		 * pre-exists, then we kill all volumeMap.put(b, v); volumeMap.put(b,
		 * v); other threads that might be writing to this block, and then
		 * reopen the file.
		 */
		public BlockWriteStreams writeToBlock(Block b, boolean isRecovery)
			...
			if (isValidBlock(b)) {
				...
				// some of the packets were not received by the client. The
				// client
			...
			try {
				...
				throw new IOException(
						"Recovery waiting for thread interrupted.");
			}
		...
		/**
		 * Retrieves the offset in the block to which the the next write will
		 * write data to.
		 */
		...
		/**
		 * Sets the offset in the block to which the the next write will write
		 * data to.
		 */
		...
		synchronized File createTmpFile(FSVolume vol, Block blk)
			...
			if (vol == null) {
				...
				if (vol == null) {
					throw new IOException("Could not find volume for block "
							+ blk);
				}
			}
		...
		public synchronized void finalizeBlock(Block b) throws IOException {
			...
			if (f == null || !f.exists()) {
				throw new IOException("No temporary file " + f + " for block "
						+ b);
			}
		...
		public synchronized void unfinalizeBlock(Block b) throws IOException {
			...
			if (delBlockFromDisk(activefile.file, getMetaFile(activefile.file,
					b), b)) {
		...
		private boolean delBlockFromDisk(File blockFile, File metaFile, Block b) {
			...
			if (!blockFile.delete()) {
				DataNode.LOG.warn("Not able to delete the block file: "
						+ blockFile);
				return false;
			} else { // remove the meta file
				if (metaFile != null && !metaFile.delete()) {
					DataNode.LOG
							.warn("Not able to delete the meta block file: "
									+ metaFile);
					return false;
				}
			}
		...
		public void validateBlockMetadata(Block b) throws IOException {
			...
			if (!f.exists()) {
				throw new IOException("Block " + b + " block file " + f
						+ " does not exist on disk.");
			}
			if (b.getNumBytes() != f.length()) {
				throw new IOException("Block " + b + " length is "
						+ b.getNumBytes()
			...
			if (!meta.exists()) {
				throw new IOException("Block " + b + " metafile " + meta
						+ " does not exist on disk.");
			}
			if (meta.length() == 0) {
				throw new IOException("Block " + b + " metafile " + meta
			...
			if (stamp != b.getGenerationStamp()) {
				throw new IOException("Block " + b + " genstamp is "
		...
		//TODO 
		/**
		 * To get the ongoingCreate to get rid of the block while coding processes finishes
		 */
		public HashMap<Block, ActiveFile>  getOngoingCreates()
		{
		...
		public void invalidate(Block invalidBlks[]) throws IOException {
			...
			for (int i = 0; i < invalidBlks.length; i++) {
				...
				DataNode.LOG.info("Deleting block " + invalidBlks[i] + " file "
						+ f);
				if (f.exists()) {
					//
					...
					//
					DataNode.LOG.info("File " + f
							+ " was deleted but still exists!");
				}
			}
			if (error) {
		...
		void registerMBean(final String storageId) {
			...
			try {
				...
				mbeanName = MBeanUtil.registerMBean(serverName,
